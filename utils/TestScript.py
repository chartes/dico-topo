...
with open("/home/corentink/Bureau/Dicotopo/Tableau_Correction/ResultatTest/DT"+ dep +".csv", "w") as csvfile:
    ListresultatTest = csv.writer(csvfile)
    ListresultatTest.writerow(["Nombre d'article", "Nombre de commune", "Code INSEE invalide", "Manque attribut type", "Manque balise insee", "Nombre article lié", "Balise commune avec attribut insee vide", "Attribut insee mal formé", "Pourcentage de réussite"])
    ListresultatTest.writerow([count_article, count_insee, count_leninsee, count_notype, count_noinsee, count_insee+count_commune_insee, count_commune_inseeempty, count_invalideinsee, ((count_insee + count_commune_insee)/count_article)*100])
